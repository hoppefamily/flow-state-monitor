"""
Optional IBKR/CapTrader data ingestion module for flow-state-monitor.
...
class IBKRDataFetcher:
    ...
    from IBKR that can be directly used with FlowStateMonitor.
    ...
    Example:
        ...
        >>> results = monitor.analyze(prices=data['prices'])
    """
    ...
    def fetch_daily_bars(
        ...
        Returns:
            Dictionary with keys: 'prices' (closes), 'opens', 'highs', 'lows'
            Each contains a list of floats ordered chronologically (oldest first)
            Note: 'borrow_rates' must be obtained from another source
        ...
        return {
            'prices': closes,  # Primary field for FlowStateMonitor
            'opens': opens,
            'highs': highs,
            'lows': lows,
        }
    ...
    def fetch_multiple_symbols(
        ...
        Example:
            ...
            >>> aapl_data = data['AAPL']
            >>> monitor.analyze(prices=aapl_data['prices'])
        """
        results = {}
...
def fetch_ibkr_data(
    ...
    **kwargs
    ...
    Returns:
        Dictionary with price data ready for FlowStateMonitor
        Note: 'borrow_rates' must be obtained from another source
    ...
    Example:
        ...
        >>>
        >>> # You need to provide borrow_rates from another source
        >>> borrow_rates = [2.5, 3.0, 5.2, ...]  # From your data provider
        >>>
        >>> # Analyze with monitor
        >>> monitor = FlowStateMonitor()
        >>> results = monitor.analyze(borrow_rates=borrow_rates, prices=data['prices'])
    """
